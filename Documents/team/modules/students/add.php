...
function generateStudentNumber($db) {
    try {
        $last = $db->query("SELECT student_number FROM students WHERE student_number IS NOT NULL ORDER BY id DESC LIMIT 1")->fetchColumn();
...
// Ensure Class 1-10 are always available if not present
for ($i = 1; $i <= 10; $i++) {
    if (!in_array("Class $i", $classes)) {
        $classes[] = "Class $i";
    }
}
sort($classes, SORT_NATURAL);

$suggestedNum = generateStudentNumber($db);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCsrfToken($_POST['csrf_token'] ?? '')) {
        $errors[] = 'Security check failed. Please refresh and try again.';
    }
    $studentNum = strtoupper(trim($_POST['student_number'] ?? $suggestedNum));
    ...
    $portalPass = $_POST['portal_password'] ?? '';

    if (!$name)       $errors[] = 'Student name is required.';
    if (!$class)      $errors[] = 'Class is required.';
    if (!$studentNum) $errors[] = 'Student ID is required.';
    if ($email && !filter_var($email, FILTER_VALIDATE_EMAIL)) $errors[] = 'Invalid email address.';

    // Check duplicate student number
    if ($studentNum) {
        $dup = $db->prepare("SELECT id FROM students WHERE student_number=?"); $dup->execute([$studentNum]);
        if ($dup->fetch()) $errors[] = "Student ID '$studentNum' already exists.";
    }

    if (!$errors) {
        $db->prepare("INSERT INTO students (student_number,name,email,phone,alt_phone,parent_name,class,batch,batch_id,mentor_id,address,dob,gender,admission_date,fee_status,fee_amount,notes,created_by) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)")
           ->execute([$studentNum,$name,$email,$phone,$altPhone,$parent,$class,$batchId?'':'',$batchId?:null,$mentorId?:null,$address,$dob?:null,$gender,$admDate,$feeStatus,$feeAmount,$notes,$user['id']]);
        $newSid = $db->lastInsertId();

        // Add to batch_students if batch selected
        if ($batchId) {
            try { $db->prepare("INSERT IGNORE INTO batch_students (batch_id,student_id) VALUES (?,?)")->execute([$batchId,$newSid]); } catch(Exception $e) {}
        }

        // Create portal password if provided
        if ($portalPass && strlen($portalPass) >= 4) {
            $hash = password_hash($portalPass, PASSWORD_BCRYPT);
            try {
                $db->prepare("INSERT INTO student_users (student_id,password,status) VALUES (?,?,'active')
                    ON DUPLICATE KEY UPDATE password=VALUES(password), status='active'")->execute([$newSid,$hash]);
            } catch(Exception $e) {}
        }

        logActivity($user['id'], "Added student: $name ($studentNum)", 'students');

        // Phase 7: Send Welcome Email
        if ($email && $portalPass) {
            try {
                // We need student_notifications.php for helper functions if needed, but email.php is already included via db.php -> auth.php -> ?
                // Actually add.php includes auth.php which might not include email.php directly.
                require_once '../../includes/email.php';
                sendWelcomeEmail($email, $name, $studentNum, $portalPass);
            } catch (Exception $e) {}
        }
...
<div class="card">
    <div class="card-header" style="flex-wrap: wrap; gap: 16px;">
        <div class="card-title" style="display:flex; align-items:center; gap:12px;">
            ...
            Add New Student
            <span class="badge" style="background:#f8fafc; color:var(--text-mid); font-family:monospace; font-size:14px; letter-spacing:1px; padding:6px 12px; border:1px solid var(--border);">ID: <span id="displayStuNum"><?= htmlspecialchars($_POST['student_number'] ?? $suggestedNum) ?></span></span>
        </div>
        <a href="index.php" class="btn btn-secondary btn-sm"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-right:2px"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg> Cancel</a>
    </div>
    <div class="card-body">
        ...
        <form method="POST" id="mainForm">
            <!-- Hidden field for auto-generated ID -->
            <?= csrfField() ?>
            <input type="hidden" name="student_number" id="hiddenStuNum" value="<?= htmlspecialchars($_POST['student_number'] ?? $suggestedNum) ?>">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <div class="input-with-icon">
                            ...
                            <input type="text" name="name" id="nameInput" value="<?= htmlspecialchars($_POST['name']??'') ?>" placeholder="Full legal name" required autofocus>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="class" id="classSelect" required>
                            ...
                            <option value="<?= htmlspecialchars($c) ?>" <?= ($_POST['class']??'')===$c?'selected':'' ?>><?= htmlspecialchars($c) ?></option>
                            <?php endforeach; ?>
                    ...
                    <div class="form-group">
                        ...
                        <select name="batch_id">
                            ...
                            <option value="<?= $b['id'] ?>" <?= ($_POST['batch_id']??0)==$b['id']?'selected':'' ?>><?= sanitize($b['name']) ?><?= $b['class']?' ('.$b['class'].')':'' ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="mentor_id">
                            ...
                            <option value="<?= $m['id'] ?>" <?= ($_POST['mentor_id']??0)==$m['id']?'selected':'' ?>><?= sanitize($m['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <div class="input-with-icon">
                            ...
                            <input type="text" name="parent_name" value="<?= htmlspecialchars($_POST['parent_name']??'') ?>" placeholder="Legal guardian name">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="phone-wrapper">
                            ...
                            <input type="tel" name="phone" value="<?= htmlspecialchars($_POST['phone']??'') ?>" placeholder="98765 43210" pattern="[0-9]{10}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="phone-wrapper">
                            <div class="phone-prefix">+91</div>
                            <input type="tel" name="alt_phone" value="<?= htmlspecialchars($_POST['alt_phone']??'') ?>" placeholder="Optional second number" pattern="[0-9]*">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-with-icon">
                            ...
                            <input type="email" name="email" value="<?= htmlspecialchars($_POST['email']??'') ?>" placeholder="student@email.com">
                        </div>
                    </div>
                    
                    <div class="form-group" style="grid-column:1/-1">
                        <label>Full Residential Address</label>
                        <textarea name="address" placeholder="e.g., Flat 4B, Example Apartments, Street Name, City" rows="3" style="resize:vertical; min-height:80px; padding:12px; border-radius:8px; border:1px solid var(--border); width:100%; box-sizing:border-box; font-family:inherit;"><?= htmlspecialchars($_POST['address']??'') ?></textarea>
                    </div>
                </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <div style="display:flex;gap:8px;align-items:center">
                            <div class="input-with-icon" style="flex:1">
                                ...
                                <input type="text" name="portal_password" id="portalPass" value="<?= htmlspecialchars($_POST['portal_password'] ?? '') ?>" placeholder="Min 4 chars (Optional)">
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="fee_status">
                            <option value="Pending" <?= ($_POST['fee_status']??'Pending')==='Pending'?'selected':'' ?>>Pending</option>
                            <option value="Paid"    <?= ($_POST['fee_status']??'')==='Paid'   ?'selected':'' ?>>Paid</option>
                            <option value="Partial" <?= ($_POST['fee_status']??'')==='Partial'?'selected':'' ?>>Partial</option>
                        </select>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-with-icon">
                            ...
                            <input type="number" name="fee_amount" value="<?= htmlspecialchars($_POST['fee_amount']??'') ?>" placeholder="0.00" min="0" step="any">
                        </div>
                    </div>
                    ...
                    <div class="form-group" style="grid-column:1/-1">
                        ...
                        <div class="textarea-wrap">
                            <textarea name="notes" id="notesField" oninput="updateCharCount()" placeholder="Any private remarks or notes regarding admission..." rows="3" style="resize:vertical; min-height:80px; padding-bottom: 24px; padding-top: 12px; padding-left: 12px; padding-right: 12px; border-radius: 8px; border: 1px solid var(--border); width: 100%; box-sizing: border-box; font-family: inherit;" maxlength="500"><?= htmlspecialchars($_POST['notes']??'') ?></textarea>
